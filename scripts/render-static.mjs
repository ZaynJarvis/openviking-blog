...
async function rewriteCopiedLlms(dir = path.join(distDir, 'post')) {
  let entries = [];
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await rewriteCopiedLlms(file);
    } else if (entry.isFile() && entry.name === 'llm.txt') {
      const source = await readFile(file, 'utf8');
      const next = source.replaceAll(DEFAULT_SITE_URL, SITE_URL);
